    ...
    for (int c = 0; c < im.channels(); ++c) {
        for (int y = 0; y < scaled.height(); ++y) {
            for (int x = 0; x < scaled.width(); ++x) {
                scaled(x, y, c) = im(floor(float(x) / factor),
                                     floor(float(y) / factor),
                                     c);
            }
        }
    }
...
float interpolateLin(const Image &im, float x, float y, int z, bool clamp) {
    ...
    vector<int> A, B, C, D;
    A = {(int)floor(x), (int)floor(y)};
    B = {(int)ceil(x), (int)floor(y)};
    C = {(int)floor(x), (int)ceil(y)};
    D = {(int)ceil(x), (int)ceil(y)};
    // Interpolate on x first.
    float valTop = (A[0] == B[0] ? im.smartAccessor(A[0], A[1], z, clamp) :
                    im.smartAccessor(A[0], A[1], z, clamp) * (B[0] - x) / (B[0] - A[0]) +
                    im.smartAccessor(B[0], B[1], z, clamp) * (x - A[0]) / (B[0] - A[0]));
    float valBottom = (C[0] == D[0] ? im.smartAccessor(C[0], C[1], z, clamp) :
                       im.smartAccessor(C[0], C[1], z, clamp) * (D[0] - x) / (D[0] - C[0]) +
                       im.smartAccessor(D[0], D[1], z, clamp) * (x - C[0]) / (D[0] - C[0]));
    // Interpolate on y and return.
    return (
               A[1] == C[1] ? valTop
               :
               valTop * (C[1] - y) / (C[1] - A[1]) +
               valBottom * (y - A[1]) / (C[1] - A[1])
           );
}
...
Image scaleLin(const Image &im, float factor) {
    ...
    for (int c = 0; c < im.channels(); ++c) {
        for (int y = 0; y < scaled.height(); ++y) {
            for (int x = 0; x < scaled.width(); ++x) {
                scaled(x, y, c) = interpolateLin(im,
                                                 (float)x / factor,
                                                 (float)y / factor,
                                                 c,
                                                 true);
