...
// This lessons demonstrates clamping, the use of multiple input pixels per
// output, and a multi-stage pipeline
...
int main(int argc, char **argv) {
  ...
  // Another stage computes dI/dy (for all three channels of RGB in both
  // cases). The final stage computes the magnitude of the corresponding
  // vector
  ...
  // comgradientpile and execute. You'll note that we subtracted 1 from the
  // width and height to make sure that the x+1 and y+1 neighbors always
  // exist. We'll see a more general solution in the next tutorial
  Image<uint8_t> output =
      gradientMagnitude.realize(im.width() - 1, im.height() - 1, im.channels());
