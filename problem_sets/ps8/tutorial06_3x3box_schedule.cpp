...
int main(int argc, char **argv) {
  ...
  //////// SCHEDULE 1 : ROOT ////////
  {
    ...
    cout << "\nSchedule 1, ROOT:\n";
    // Func::print_loop_nest prints out a pseudo code of the schedule which
  ...
  //////// SCHEDULE 2 : INLINE ////////
  {
    ...
    // reciprocal.
    // The reciprocal probably would stay in register, making everything
    // blazingly fast.
    // Compilers can be pretty smart.

    blur_y.compile_to_lowered_stmt("Output/blur.html", {}, HTML);
  }
  ...
  //////// SCHEDULE 3 : TILING and INTERLEAVING ////////
  {
    ...
    // redundancy
    // than inline. It still has some redundancy because of the enlargement at
    // tile
    // boundaries
  ...
  //////// SCHEDULE 3.5 : TILING and INTERLEAVING, with different memory
  /// allocation strategies ////////
  {
    ...
    // We perform the tiling as in the previous example
    blur_y.tile(x, y, xo, yo, xi, yi, 256, 32); // compute in tiles of 256x32

    // Instead of reallocating the blur_x array every time at xo, we could also
    // preallocate the memory at yo, and reuse it for every xo.
    // This is a slower strategy in this particular case, since the size of
    // blur_x array is just 3
    // and it's better to fit it in the registers.
    // For larger kernel size this could become handy
    blur_x.store_at(blur_y, yo).compute_at(blur_y, xo);
  ...
  //////// SCHEDULE 4 : TILING, INTERLEAVING, and PARALLELISM ////////
  {
    ...
    // vectorization
    // For example, if you specify a width larger than what your processor can
    // achieve,
    // it won't work
    ...
    // Note that blur_x gets vectorized at x whereas blur_y's vectorize was
    // called with xi
    // This is because blur_x does not have a notion of x_i. Its tiling
    // piggybacked on that
    // of blur_y, and as far as blur_x is concerned, it just gets called for a
    // tile and has
    // a single granularity of x and y for this tile (although of course
    // vectorize then adds
    // a second.
    ...
    // the equivalent C++ code would be similar as above but with a parallel y
    // loop
    // and a modified inner xi loop. In effect, vectorization adds an extra
    // level of nesting
    // in strides of 8 but unrolls the innermost level into single vector
    // instructions
  }
...
// Report the runtime and throughout for the above 4 schedules in the submission
// form.
