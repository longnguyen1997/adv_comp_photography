...
// the simple halide_image.h version. Also now includes PPM support for faster
// load/save.
...
inline bool ends_with_ignore_case(const std::string &ac,
                                  const std::string &bc) {
...
struct PngRowPointers {
  PngRowPointers(int height, int rowbytes)
      : p(new png_bytep[height]), height(height) {
    if (p != nullptr) {
...
bool load_png(const std::string &filename, ImageType *im) {
  ...
  if (!check(f.f != nullptr, "File %s could not be opened for reading\n",
             filename.c_str()))
    return false;
  if (!check(fread(header, 1, 8, f.f) == 8,
             "File ended before end of header\n"))
    return false;
  if (!check(!png_sig_cmp(header, 0, 8),
             "File %s is not recognized as a PNG file\n", filename.c_str()))
    return false;
  ...
  // Expand low-bpp images to have only 1 pixel per byte (As opposed to tight
  // packing)
  if (bit_depth < 8) {
  ...
  Internal::PngRowPointers row_pointers(im->height(),
                                        png_get_rowbytes(png_ptr, info_ptr));
  png_read_image(png_ptr, row_pointers.p);

  if (!check((bit_depth == 8) || (bit_depth == 16),
             "Can only handle 8-bit or 16-bit pngs\n"))
    return false;

  // convert the data to ImageType::ElemType

  int c_stride = (im->channels() == 1) ? 0 : im->stride(2);
  typename ImageType::ElemType *ptr =
      (typename ImageType::ElemType *)im->data();
  if (bit_depth == 8) {
...
bool save_png(ImageType &im, const std::string &filename) {
  ...
  if (!check(im.channels() > 0 && im.channels() < 5, "Can't write PNG files "
                                                     "that have other than 1, "
                                                     "2, 3, or 4 channels\n"))
    return false;
  ...
  if (!check(!setjmp(png_jmpbuf(png_ptr)),
  ...
  png_set_IHDR(png_ptr, info_ptr, im.width(), im.height(), bit_depth,
  ...
  int c_stride = (im.channels() == 1) ? 0 : im.stride(2);
  typename ImageType::ElemType *srcPtr =
      (typename ImageType::ElemType *)im.data();
...
bool load_ppm(const std::string &filename, ImageType *im) {
  ...
  if (maxval == 255) {
    ...
    if (!check(false, "Invalid bit depth in PPM\n"))
      return false;
  }

  if (!check(header == std::string("P6") || header == std::string("p6"),
  ...
  *im = ImageType(width, height, channels);
  ...
  if (bit_depth == 8) {
    ...
    if (!check(fread((void *)data.data(), sizeof(uint8_t), width * height * 3,
                     f.f) == (size_t)(width * height * 3),
               "Could not read PPM 8-bit data\n"))
      return false;
    typename ImageType::ElemType *im_data =
        (typename ImageType::ElemType *)im->data();
    for (int y = 0; y < im->height(); y++) {
    ...
    if (!check(fread((void *)data.data(), sizeof(uint16_t), width * height * 3,
                     f.f) == (size_t)(width * height * 3),
...
bool save_ppm(ImageType &im, const std::string &filename) {
  ...
  if (bit_depth == 8) {
    ...
    if (!check(fwrite((void *)data.data(), sizeof(uint8_t), width * height * 3,
                      f.f) == (size_t)(width * height * 3),
    ...
    if (!check(fwrite((void *)data.data(), sizeof(uint16_t), width * height * 3,
                      f.f) == (size_t)(width * height * 3),
...
bool load(const std::string &filename, ImageType *im) {
  if (Internal::ends_with_ignore_case(filename, ".png")) {
    ...
    return check(false,
                 "[load] unsupported file extension (png|ppm supported)");
  }
}
...
bool save(ImageType &im, const std::string &filename) {
  if (Internal::ends_with_ignore_case(filename, ".png")) {
    ...
    return check(false,
                 "[save] unsupported file extension (png|ppm supported)");
  }
}
...
public:
  ...
  template <typename ImageType> inline operator ImageType() {
    ImageType im;
